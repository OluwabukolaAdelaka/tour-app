...
const Carousel = () => {
  ...
  return (
    <div className='relative max-w-[1240px] mx-auto py-16 p-4 flex justify-center'>
        <BsArrowLeftSquareFill onClick={prevSlide} size={30} className='top-[50%] absolute text-white left-8'/>
        <BsArrowRightSquareFill onClick={nextSlide} size={30} className='top-[50%] absolute text-white right-8'/>
        {sliderData.map((item, index) => (
            <div key={index} className={index === slide ? 'opacity-100' : 'opacity-0'}>
                {index === slide && <img src={item.url} alt="carousel" className='rounded-md w-full'/>} 
            </div>
        ))}
    </div>
  )
